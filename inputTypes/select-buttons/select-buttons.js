...
AutoForm.addInputType("select-buttons", {
    ...
    contextAdjust: function(context) {
        ...
        if(context.atts.allowFilter) {
            //console.info("allowing filter");
        }

        //console.debug("Setting context",context,this);
        if(context.atts.singleSelection) {
            this.singleSelection = context.atts.singleSelection;
        }

        var colSizeClass = "col-sm-6 col-md-4";
        if(context.atts.colSizeClass) {
            colSizeClass = context.atts.colSizeClass;
        }
...
Template.afSelectButtons.helpers({
    ...
    currentValues: function() {
        ...
        if(tpl.firstNode!=null) {
            values = $(tpl.firstNode).find("button.btn_item.active").map(function() {
                return this.value;
            }).get();
        }
